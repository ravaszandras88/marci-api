...
export async function POST(request: NextRequest) {
  try {
    const { customerEmail, customerName, successUrl, cancelUrl } = await request.json();
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: successUrl || `${request.headers.get('origin')}/mentoring/success?session_id={CHECKOUT_SESSION_ID}`,
      cancel_url: cancelUrl || `${request.headers.get('origin')}/#services`,
      metadata: {
